...
OFFLINE_FILE=""
OS_SUPPORT="ubuntu20.04 ubuntu20.10 ubuntu21.04 ubuntu22.04 centos7 centos8"
...
$(
    ...
    for h in $api_servers; do
      echo "    server apiserver${index} $h:6443 check"
      index=$((index + 1))
    done
  )
EOF
...
function check::preflight() {
  ...
  # check docker cri
  [[ "${KUBE_CRI}" == "docker" && ("${KUBE_VERSION}" == "latest" || "${KUBE_VERSION}" =~ 1.24) ]] && {
    log::error "[limit]" "$KUBE_CRI is not supported, only [containerd,cri-o]"
    exit 1
  }
}
...
function cert::renew_node() {
  ...
  for host in ${hosts}; do
    ...
    command::exec "${MGMT_NODE}" "kubectl drain $host --force --ignore-daemonsets  --delete-emptydir-data"
...
function init::node_config() {
  ...
  for host in $MASTER_NODES; do
   ...
   "
    ...
    # 设置主机名和解析
    command::exec "${host}" "printf \"\\n${MGMT_NODE_IP} $KUBE_APISERVER\\n$node_hosts\" >> /etc/hosts"
    if [[ "${RENAME_NODES:-}" == "1" ]]; then
      command::exec "${host}" "hostnamectl set-hostname ${HOSTNAME_PREFIX}-master-node${master_index}"
    fi
...
EOF
  ...
  for host in $WORKER_NODES; do
    ...
    "
    if [[ "${RENAME_NODES:-}" == "1" ]]; then
      command::exec "${host}" "hostnamectl set-hostname ${HOSTNAME_PREFIX}-worker-node${worker_index}"
    fi
...
function init::node() {
  ...
  for h in $MASTER_NODES; do
    if [[ "${RENAME_NODES:-}" == "1" ]]; then
      ...
      command::exec "${h}" "hostnamectl --static"
...
"
  ...
  command::exec "${MGMT_NODE}" "kubeadm init --config=/etc/kubernetes/kubeadmcfg.yaml --upload-certs --ignore-preflight-errors=Port-10250,DirAvailable--var-lib-etcd"
...
function add::network() {
  ...
  if [[ "$KUBE_NETWORK" == "flannel" ]]; then
    ...
    utils::download_file "${GITHUB_PROXY}https://raw.githubusercontent.com/projectcalico/calico/v${CALICO_VERSION}/manifests/calico.yaml" "${OFFLINE_DIR}/manifests/calico.yaml"
    utils::download_file "${GITHUB_PROXY}https://raw.githubusercontent.com/projectcalico/calico/v${CALICO_VERSION}/manifests/calicoctl.yaml" "${OFFLINE_DIR}/manifests/calicoctl.yaml"
    command::exec "${MGMT_NODE}" "
...
function del::node() {
  ...
  for host in $MASTER_NODES $WORKER_NODES; do
    ...
    if [[ "${node_name}" == "" ]]; then
      ...
      command::exec "${MGMT_NODE}" "kubectl drain $node_name --force --ignore-daemonsets --delete-emptydir-data"
...
function transform::nodes {
  # 使用tr命令将空格替换为换行符，并将冒号替换为空格，以便于使用read进行逐行读取。
  # 每次循环读取一对键值，并将它们存储到map关联数组中。最后，遍历并打印map中的所有键值对。
  # 设置IFS为分隔符（这里是空格和冒号）
  IFS=' '

  local MASTER_NODES_IP=""
...
Flag:
  ...
  -renameNodes         rename nodes with prefix name default:1
...
Example:
  ...
  [init cluster and rename node with prefix name]
  $0 init \\
  --master 192.168.77.130,192.168.77.131,192.168.77.132 \\
  --worker 192.168.77.133,192.168.77.134,192.168.77.135 \\
  --user root \\
  ...
  [reset cluster by hand]
  $0 resetbyhand \\
  --user root \\
  --password 123456  
  --master 192.168.18.52   --version 1.28
...
if [[ "${INIT_TAG:-}" == "1" ]]; then
  ...
  #script::upgrage_kube 'init' $KUBE_VERSION
  upgrade::cluster
